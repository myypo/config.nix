...
var WYTAS_ver = "4.5.2";
console.log(
  "WYTAS:YouTube Ad Skipper by Vyasdev217 (https://greasyfork.org/en/users/1016463)",
);
const observer = new MutationObserver((mutations) => {
  ...
  mutations.forEach((mutation) => {
    if (
      document.contains(document.getElementsByClassName("ytp-preview-ad")[0])
    ) {
      console.log("WYTAS:Detected ad (action " + WYTAS_ver + "_1)");
      document.getElementsByClassName(
        "html5-video-container",
      )[0].children[0].currentTime = 250;
    }
    if (
      document.contains(
        document.getElementsByClassName("ytp-skip-ad-button__text")[0],
      )
    ) {
      console.log("WYTAS:Detected ad (action " + WYTAS_ver + "_2)");
...
pbs.type = "number";
pbs.style =
  "background-color: rgba(0, 0, 0, 0.5); color: white; border: none; cursor: pointer; overflow: hidden; outline: none; width: 5ch; text-align: center; font-size: auto; -webkit-appearance: textfield; -moz-appearance: textfield;";
pbs.title = "Playback speed";
var incrementButton = document.createElement("BUTTON");
incrementButton.textContent = "+";
incrementButton.style =
  "background-color: rgba(0, 0, 0, 0.5); color: white; border: none; cursor: pointer; outline: none; font-size: 1.5em;";
incrementButton.addEventListener("click", function () {
  pbs.stepUp();
  var event = new Event("change", { bubbles: true, cancelable: true });
  pbs.dispatchEvent(event);
});
